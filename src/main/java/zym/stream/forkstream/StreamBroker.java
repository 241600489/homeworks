...
/**
 * @Author unyielding
 * @date 2018/7/22 0022 19:44
 * @desc 对原始的流进行封装, 在此基础上可以继续定义你希望执行的各种操作
 */
public class StreamBroker<T> {
    private final Stream<T> stream;

    private final Map<Object, Function<Stream<T>, ?>> forks =
            new HashMap<>();

    public StreamBroker(Stream<T> stream) {
        this.stream = stream;
    }

    public StreamBroker<T> fork(Object key, Function<Stream<T>, ?> f) {
        forks.put(key, f);
        return this;
    }
    ...
    private ForkingStreamConsumer<T> build() {
        ...
        HashMap<Object, Future<?>> actions = forks.entrySet()
                .stream()
        ...
        return new ForkingStreamConsumer<T>(queues, actions);
    }
